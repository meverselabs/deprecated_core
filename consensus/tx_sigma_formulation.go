...
func init() {
	data.RegisterTransaction("consensus.SigmaFormulation", func(t transaction.Type) transaction.Transaction {
		return &SigmaFormulation{
			Base: transaction.Base{
				Type_: t,
			},
		}
	}, func(loader data.Loader, t transaction.Transaction, signers []common.PublicHash) error {
		tx := t.(*SigmaFormulation)
...
// SigmaFormulation is a consensus.SigmaFormulation
// It is used to make formulation account
type SigmaFormulation struct {
	transaction.Base
	Seq_             uint64
...
func (tx *SigmaFormulation) IsUTXO() bool {
	return false
}

// From returns the creator of the transaction
func (tx *SigmaFormulation) From() common.Address {
	return tx.From_
}

// Seq returns the sequence of the transaction
func (tx *SigmaFormulation) Seq() uint64 {
	return tx.Seq_
}

// Hash returns the hash value of it
func (tx *SigmaFormulation) Hash() hash.Hash256 {
	return hash.DoubleHashByWriterTo(tx)
}

// WriteTo is a serialization function
func (tx *SigmaFormulation) WriteTo(w io.Writer) (int64, error) {
	var wrote int64
...
func (tx *SigmaFormulation) ReadFrom(r io.Reader) (int64, error) {
	var read int64
...
func (tx *SigmaFormulation) MarshalJSON() ([]byte, error) {
